...
eta_inv_mass = et.inv_mass_recon_list(eta_mu_1_vec, eta_mu_2_vec, eta_mu_3_vec, eta_mu_4_vec)
eta_prime_inv_mass = et.inv_mass_recon_list(PMuVec1, PMuVec2, PMuVec3, PMuVec4)

et.hist_mass_curve(eta_inv_mass, 'eta_inv_mass', 'm_4$\mu$ Invariant Mass Eta', 'teal', 200)
et.hist_mass_curve(eta_prime_inv_mass,'eta_prime_inv_mass', 'm_4$\mu$ Invariant Mass Eta Prime', 'rebeccapurple', 200 )
# Histogram 1 pT
muons_pt = et.LApt(eta_mu_1_vec, eta_mu_2_vec, eta_mu_3_vec, eta_mu_4_vec)
et.hist(muons_pt, '4Muons_pT', 'pT Among All Muons', 'Energy (GeV)', 'Number of Particles per pT', 'teal', 300)

# Histogram 2 all 4 muons 
muons_eta = et.LAeta(eta_mu_1_vec, eta_mu_2_vec, eta_mu_3_vec, eta_mu_4_vec)
et.hist(muons_eta, '4MuonsEta', 'eta Among Individual Muons', 'eta', 'Number pf particle per eta','teal',  200 )
...
print('The average pT among all muons from eta is',muons_pt.mean(),'GeV')
...
et.hist(prime_muons_pt, 'prime_4Muons_pT', 'pT Among Muons from Eta Prime', 'Energy (GeV)', 'Number of Particles per pT', 'rebeccapurple', 200)

# Prime Histogram 2' all 4 muons in acceptance
prime_muons_eta = et.LAeta(PMuVec1, PMuVec2, PMuVec3, PMuVec4)
et.hist(prime_muons_eta, 'prime_4Muons_eta','Eta Among Individual Muons from Eta Prime', 'eta' ,'Number of Particles per eta', 'rebeccapurple', 200 )
...
for i in range(len(prime_sibmuons[:,1])):
    ...
    if sibs[0] <= 5 and sibs[0] >= 2 and sibs[1] <= 5 and sibs[1] >= 2 and sibs[2] <= 5 and sibs[2] >= 2 and sibs[3] <= 5 and sibs[3] >= 2:
        counter += 1
        if counter == 1:
            prime_sibs_eta_acc = prime_sibmuons[i,:]
        else:
            prime_sibs_eta_acc = np.vstack((prime_sibs_eta_acc,sibs))
et.hist(prime_delta_eta_list, 'prime_4muons_delta_eta', 'Delta Eta Among Sibling Muons From Eta Prime', 'Delta Eta', 'Number of Particles per Delta eta', 'rebeccapurple', 100) 
...
et.hist(prime_delta_eta_acc_list, 'prime_4muons_acc_delta_eta', 'Delta Eta Among Sibling Muons (all in acceptance) From Eta Prime', 'Delta Eta', 'Number of Particles per Delta eta', 'rebeccapurple', 100)
...
et.hist(all_muons_acc_pt, '4muons_acc_pT', 'Muons (all in acceptance) pT', 'pT in GeV','Number of Particles per pT', 'teal', 65 )


# # New Histogram
all_prime_muons_acc_pt = et.eta_acc_LApt(PMuVec1, PMuVec2, PMuVec3, PMuVec4)
et.hist(all_prime_muons_acc_pt, 'prime_4muons_acc_pT', 'Eta_Prime Muons (all in acceptance) pT','pT in GeV', 'Number of Particles per pT', 'rebeccapurple', 100)

#NewHistogram

print('The Highest pT among all muons from eta_prime is', prime_muons_pt.max(),'GeV')
print('The average pT among all muons from eta_prime is', prime_muons_pt.mean(),'GeV')
...
et.hist_pT_4mu('4muons_mean_pT', 'Eta Muon Siblings in Acc pT Spread', muon_sorted_pt)
et.hist_pT_4mu('prime4muons_mean_pT', 'Eta_Prime Muon Siblings in Acc pT Spread', muon_prime_sorted_pt)
